...
### 内置功能
- **数据大厅**(`/center`):登录后落地页,业务模块以苹果风格方格呈现(注册表驱动,新增功能只需追加一条即可扩展入口)。
...
- **个人健康信息管理**(`/health`):健康事件的聚合记录系统——以「事件」为原子(就诊、检查项、自测、生理事件……一切皆事件),统一字段:人/时间/类型(必填)+ 项目/科属/机构/接诊医师/结论/详述/原始凭证(可选)。
  - **双链关联**:详述中的 `[[事件ID]]` 文本链接是关联的唯一来源,由「插入事件链接」选择器插入——可直接选择已有事件,也可在插入时**新建关联事件**(与完整新增流程同表单);系统自动维护「被关联」列表(纯入链视图,附引用上下文片段)。
  - **归集页**(`/health/summary`):按人/时间/科属筛选,事件按时间正序呈现,详述中的关联可页内展开,支持复制全文与打印排版,供就诊与 AI 自诊使用。
  - **重建索引**(`/health` 右上角):按详述文本链接全库重算「被关联」列表,修复历史遗留脏数据(带确认与结果提示)。
...
4. 启动后,使用管理员凭据登录 PocketBase 管理后台 `http://localhost:8090/_/`,进入 Settings → Import collections → Load from JSON file,选择 [backend/pb_schema.json](backend/pb_schema.json) 导入(users、persons、events、tools_settings、health_events 等集合)。
