...
#   - 产物保存至 release/tans-PIM-<os>-<arch>-<版本>[.exe](归档)
#   - 成功后自动复制到项目根目录 tans-pim[.exe](运行使用相对路径 db/)
...
OUT_NAME="tans-PIM-${GOOS}-${GOARCH}-${VERSION}${EXT}"
...
# 6. 复制到项目根目录(与现有 tans-pim 相同位置,运行使用相对路径 db/)
DEST_PATH="$ROOT_DIR/tans-pim${EXT}"
# 先复制到临时名再原子 mv:服务运行中二进制被占用,直接 cp 覆盖会报"文本文件忙"
TMP_DEST="$ROOT_DIR/.tans-pim.tmp${EXT}"
