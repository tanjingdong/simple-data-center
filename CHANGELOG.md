...
- 社交模块的事件模型变更：采用文本描述+嵌入多个参与方的数据模型，即1个事件=通过文本描述的A和B的关系，从而实现了：在1个事件中可以看到多个参与方的关系，并在多个对象中可以自动检索到同一个事件。
- 针对模型变更的新的前端：引入Tiptap，可在录入事件中@联系人，并渲染为链接。联系人详情、事件显示时可点击跳转到其他联系人详情、事件页面。
- 优化操作体验：可在联系人详情页面直接点击新增事件。
- 优化操作体验：在详情页面显示该人事件。
